...
The simple user friendly interface of the applcation allows a user to first mint and register his/her art piece. Once the NFT is minted and registered, we utilize Pinata services to store the srtwork files and related metadata in a decentralized manner. </br>
After minting and registering their art, the creators can right away auction it on the decentralized marketplace.

---

## ![Digital Art](Images/sc.png)
...
- Auction:</br>
  ...
  - During auction period interested parties can place their bids through the bid function in the auction contract. When a bid is placed the auction contract requires that the balance of the bidder is higher than the bid to ensure that the bidder has the funds to purchase the artwork. Please note the contract forbids the seller of the art to place bids to avoid the price hiking.
  - During the auction and some time after the auction ends bidders can also withdraw their bids and receive back their funds. For this the auction contract requires that the withdrawing bidder is not the heighest bidder.

...
###
    ...
    if register.button("Register Artwork"):
        #if 'my_list' not in register.session_state:
        #    register.session_state = my_list=[]
        my_list=[]
        artwork_ipfs_hash = pin_artwork(artwork_name, file)
        artwork_uri = f"ipfs://{artwork_ipfs_hash}"
        image_ipfs_hash = pin_image(file)
        
        #st.write('address2',address)
        ...
        st.write(art_dict)
        st.write('before button',my_list)

        auc_list=register.checkbox('add NFT to list?')
        
        st.write(auc_list)

        if auc_list:
            my_list.append(art_dict)
            st.write('after button',my_list)
            #st.write(my_list)
